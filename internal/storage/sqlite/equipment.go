...
func (s *Store) CheckoutSnapshot(ctx context.Context, query queryRower, equipmentID, studentID, sessionID int64, now time.Time) (CheckoutSnapshot, error) {
	...
	var count int
	if err := query.QueryRowContext(ctx, `SELECT COUNT(*) FROM enrollments WHERE student_id=? AND session_id=? AND status='confirmed'`, studentID, sessionID).Scan(&count); err != nil {
		return out, err
	}
	out.Enrolled = count == 1
	if err := query.QueryRowContext(ctx, `SELECT COUNT(*) FROM equipment_inspections WHERE equipment_id=? AND outcome='passed' AND valid_until>?`, equipmentID, timeText(now)).Scan(&count); err != nil {
		return out, err
	}
...
func (s *Store) CurrentCheckoutSnapshot(ctx context.Context, equipmentID, studentID, sessionID int64, now time.Time) (CheckoutSnapshot, error) {
...
func (s *Store) AcquireEquipment(ctx context.Context, tx *sql.Tx, snapshot CheckoutSnapshot, actorID int64) (domain.EquipmentLoan, error) {
	...
	return s.CreateEquipmentLoan(ctx, tx, snapshot, actorID)
}

func (s *Store) CreateEquipmentLoan(ctx context.Context, tx *sql.Tx, snapshot CheckoutSnapshot, actorID int64) (domain.EquipmentLoan, error) {
	now := s.now()
