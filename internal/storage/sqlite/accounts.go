...
func (s *Store) CreateSession(ctx context.Context, accountID int64, tokenHash string, expires time.Time) (int64, error) {
	result, err := s.db.ExecContext(ctx, `INSERT INTO sessions(token_hash,account_id,expires_at,created_at) VALUES(?,?,?,?)`, tokenHash, accountID, timeText(expires), timeText(s.now()))
	if err != nil {
