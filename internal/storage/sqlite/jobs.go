...
func (s *Store) ClaimJobs(ctx context.Context, limit int, now time.Time) ([]domain.Job, error) {
	...
	err := s.InTx(ctx, func(tx *sql.Tx) error {
		rows, err := tx.QueryContext(ctx, `SELECT id,kind,job_key,payload,status,attempts,max_attempts,available_at,last_error,locked_at,created_at,updated_at FROM worker_jobs WHERE status IN ('pending','retry') AND available_at<=? ORDER BY available_at,id LIMIT ?`, timeText(now), limit)
		if err != nil {
			return err
		}
		defer rows.Close()
		...
		for i := range jobs {
			result, err := tx.ExecContext(ctx, `UPDATE worker_jobs SET status='running',attempts=attempts+1,locked_at=?,updated_at=? WHERE id=? AND status IN ('pending','retry')`, timeText(now), timeText(now), jobs[i].ID)
			if err != nil {
				return err
			}
