...
func (s *Store) enrollmentSnapshot(ctx context.Context, query snapshotQuery, studentID, sessionID int64, at time.Time) (EnrollmentSnapshot, error) {
	...
	var count int
	err = query.QueryRowContext(ctx, `SELECT COUNT(*) FROM guardian_consents WHERE student_id=? AND scope='sports_participation' AND granted_at<=? AND expires_at>? AND revoked_at IS NULL`, studentID, timeText(at), timeText(out.Session.EndsAt)).Scan(&count)
	if err != nil {
		return out, err
	}
	out.ConsentValid = count > 0
	err = query.QueryRowContext(ctx, `SELECT COUNT(*) FROM course_prerequisites p WHERE p.template_id=? AND NOT EXISTS(SELECT 1 FROM certifications c WHERE c.student_id=? AND c.sport=p.required_sport AND c.level>=p.required_level AND (c.expires_at IS NULL OR c.expires_at>?))`, out.Template.ID, studentID, timeText(out.Session.StartsAt)).Scan(&count)
	if err != nil {
		return out, err
	}
	out.PrerequisitesMet = count == 0
	err = query.QueryRowContext(ctx, `SELECT COUNT(*) FROM coach_qualifications q WHERE q.coach_id=? AND q.sport=? AND q.level>=? AND q.status='active' AND q.valid_from<=? AND q.valid_until>=?`, out.Session.CoachID, out.Template.Sport, out.Template.Level, timeText(out.Session.StartsAt), timeText(out.Session.EndsAt)).Scan(&count)
	if err != nil {
		return out, err
	}
...
func (s *Store) EnrollmentSnapshot(ctx context.Context, tx *sql.Tx, studentID, sessionID int64, at time.Time) (EnrollmentSnapshot, error) {
	return s.enrollmentSnapshot(ctx, tx, studentID, sessionID, at)
}
func (s *Store) CurrentEnrollmentSnapshot(ctx context.Context, studentID, sessionID int64, at time.Time) (EnrollmentSnapshot, error) {
...
func (s *Store) ReserveEnrollment(ctx context.Context, tx *sql.Tx, studentID int64, snapshot EnrollmentSnapshot, key string) (domain.Enrollment, error) {
	...
	return s.CreateEnrollment(ctx, tx, studentID, snapshot, key)
}

func (s *Store) CreateEnrollment(ctx context.Context, tx *sql.Tx, studentID int64, snapshot EnrollmentSnapshot, key string) (domain.Enrollment, error) {
	now := s.now()
