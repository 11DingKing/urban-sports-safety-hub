...
func (r *Runner) RunOnce(ctx context.Context) {
	...
	for _, job := range jobs {
		...
		if !ok {
			...
			attemptCtx, cancel := context.WithTimeout(executionCtx, 30*time.Second)
			err = handler(attemptCtx, job)
			cancel()
		}
		if finishErr := r.repo.FinishJob(executionCtx, job.ID, err, r.now()); finishErr != nil {
			r.logger.Error("job finalization failed", "job_id", job.ID, "error", finishErr)
		} else if err != nil {
			r.logger.Warn("job attempt failed", "job_id", job.ID, "attempt", job.Attempts, "error", err)
		}
	}
}
