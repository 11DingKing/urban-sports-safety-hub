...
func (s *Service) Enroll(ctx context.Context, principal domain.Principal, requestID string, req Request) (domain.Enrollment, error) {
	...
	snapshot, err := s.store.CurrentEnrollmentSnapshot(ctx, req.StudentID, req.SessionID, s.now())
	if err != nil {
		return domain.Enrollment{}, err
	}
	if snapshot.Session.Status != "scheduled" || !snapshot.Session.StartsAt.After(s.now()) {
		return domain.Enrollment{}, domain.NewError(domain.KindConflict, "session_closed", "course session is not open for enrollment")
	}
	...
	if age < snapshot.Template.MinimumAge {
		return domain.Enrollment{}, domain.NewError(domain.KindForbidden, "minimum_age_not_met", "student does not meet the minimum age")
	}
	if domain.IsMinor(snapshot.Student.BirthDate, snapshot.Session.StartsAt) && !snapshot.ConsentValid {
		return domain.Enrollment{}, domain.NewError(domain.KindForbidden, "consent_required", "active guardian consent is required")
	}
	if !snapshot.PrerequisitesMet {
		return domain.Enrollment{}, domain.NewError(domain.KindForbidden, "prerequisite_missing", "course prerequisite certification is missing")
	}
