...
func (s *Service) Checkout(ctx context.Context, p domain.Principal, requestID string, req CheckoutRequest) (domain.EquipmentLoan, error) {
	...
	snapshot, err := s.store.CurrentCheckoutSnapshot(ctx, req.EquipmentID, req.StudentID, req.SessionID, s.now())
	if err != nil {
		return domain.EquipmentLoan{}, err
	}
	if snapshot.Equipment.Status != "available" {
		return domain.EquipmentLoan{}, domain.NewError(domain.KindConflict, "equipment_unavailable", "equipment is not available")
	}
	if !snapshot.InspectionValid {
		return domain.EquipmentLoan{}, domain.NewError(domain.KindForbidden, "inspection_expired", "equipment inspection is missing or expired")
	}
	if !snapshot.Enrolled {
		return domain.EquipmentLoan{}, domain.NewError(domain.KindForbidden, "student_not_enrolled", "student is not enrolled in this session")
	}
	if snapshot.Equipment.Sport != string(snapshot.CourseSport) {
		return domain.EquipmentLoan{}, domain.NewError(domain.KindInvalid, "equipment_sport_mismatch", "equipment is not suitable for the session sport")
	}
