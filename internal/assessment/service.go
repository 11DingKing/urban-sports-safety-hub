...
func (s *Service) Publish(ctx context.Context, p domain.Principal, requestID string, req PublishRequest) (domain.Assessment, error) {
	...
	err = s.audit.RecordStandalone(ctx, p.AccountID, requestID, "assessment", result.ID, "assessment.published", "success", map[string]any{"status": result.Status, "score": req.Score})
	return result, err
}
